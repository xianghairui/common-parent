 ...
 * Description: 工具类
 * <P>
 * 专门来实现Server-Side与Client-Side的SSLSocket 连接
 * TODO 部分参数添加可配置
 * </p>
 * <p>
 * </p>
...
public class SSLTrustManager implements javax.net.ssl.TrustManager, javax.net.ssl.X509TrustManager, HostnameVerifier {
	...
	private static KeyStoreOptions clientStore;
	private static KeyStoreOptions serverStore;
	...
	public static HttpURLConnection connectTrustAllServer(String strUrl, Proxy proxy) throws Exception {
		...
		javax.net.ssl.SSLContext sc = javax.net.ssl.SSLContext.getInstance(SSLTrustManager.TLS);
	 ...
	 */
	public static HttpURLConnection connectProxyTrustCA(String strUrl, Proxy proxy)
		...
		KeyStore serverKeyStore = KeyStore.getInstance(SSLTrustManager.JKS);
		serverKeyStore.load(new FileInputStream(clientKeyStoreFile), clientKeyStorePwd.toCharArray());

		KeyStore serverTrustKeyStore = KeyStore.getInstance(SSLTrustManager.JKS);
		...
		SSLContext sslContext = SSLContext.getInstance(SSLTrustManager.TLS);
	 ...
	 * @throws KeyManagementException
	 */
	public static HttpURLConnection connectProxyTrustCA2(String strUrl, Proxy proxy)
			throws KeyStoreException, NoSuchAlgorithmException, CertificateException, FileNotFoundException,
			IOException, UnrecoverableKeyException, KeyManagementException {
		HttpsURLConnection.setDefaultHostnameVerifier(new HostnameVerifier() {
		...
		SSLContext sslContext = SSLContext.getInstance(SSLTrustManager.JKS);
	 ...
	 * @throws KeyManagementException
	 */
	public SSLSocket createTlsConnect(Socket socketClient)
			throws KeyStoreException, NoSuchAlgorithmException, CertificateException, FileNotFoundException,
			IOException, UnrecoverableKeyException, KeyManagementException {

		String protocol = SSLTrustManager.TLS;
		...
		KeyStore keyStore = KeyStore.getInstance(SSLTrustManager.JKS);
		keyStore.load(new FileInputStream(serverKey), serverKeyPwd.toCharArray());

		KeyStore tks = KeyStore.getInstance(SSLTrustManager.JKS);
		...
		SSLSocket clientSSLSocket = (SSLSocket) sslSocketFactory.createSocket(socketClient,
				socketClient.getInetAddress().getHostAddress(), socketClient.getPort(), true);
	...
	/**
	 * 用于单向认证 server侧只需要自己的keystore文件，不需要truststore文件
	 * client侧不需要自己的keystore文件，只需要truststore文件（其中包含server的公钥）。
	 * 此外server侧需要在创建SSLServerSocket之后设定不需要客户端证书：setNeedClientAuth(false)
	 * 
	 * @param socketClient
	 ...
	 * @throws KeyManagementException
	 */
	public static SSLSocket createTlsConnect2(Socket socketClient)
			throws KeyStoreException, NoSuchAlgorithmException, CertificateException, FileNotFoundException,
			IOException, UnrecoverableKeyException, KeyManagementException {
	 ...
	 * 
	 * 服务端使用的时候是把普通的socket转为sslsocket,并且作为服务器套接字（注意：指的不是ServerSocket,
	 * 当然ServerSocket的本质也是普通socket）
	 * 
	 * @param remoteHost
	 * @param isClient
	 * @return
	...
	public static SSLSocket getTlsTrustAllSocket(Socket remoteHost, boolean isClient) {
		...
		try {
			remoteSSLSocket = (SSLSocket) context.getSocketFactory().createSocket(remoteHost,
					remoteHost.getInetAddress().getHostName(), remoteHost.getPort(), true);
	 ...
	 * 
	 * @param isClient
	 *            是否生成客户端SSLContext，否则生成服务端SSLContext
	 * @return
	 */
	public static SSLContext getTrustAllSSLContext(boolean isClient) {
		...
		try {
			...
			javax.net.ssl.TrustManager tm = new SSLTrustManager();
			trustAllCerts[0] = tm;
			sc = javax.net.ssl.SSLContext.getInstance(protocol);
